...
// 다른 객체가 username과 socket을 수정할 일이없어 setter가 필요없으므로 getter만 사용
@Getter
...
//		roomList.add(room);
	...
	private void requestMapping(String requestJson) {
		...
		case "createRoom":
			...
			sendToAll(refeshRoomList(), connectedSocketList);
			break;
	...
	private void checkUsername(String username) {
		...
		connectedSocketList.add(this);
		sendToMe(new ResponseDto<String>("usernameCheckSuccessfully", null)); // 이때 응답이 오면 cardLayout을 바꿔줌
		sendToMe(refeshRoomList());
	}

	private ResponseDto<List<Map<String, String>>> refeshRoomList() {
